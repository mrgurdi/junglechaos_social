...
var MultiplayerWaitScreen = Scene.$extend({
	__init__:function(w,h,game)
	{
		...
		AppWarpHandler.connect(WARP_API_KEY,WARP_SECRETE_KEY,FB_ID,this);
	},
	...
	onJoined: function(user)
	{
		...
		if(user)
		{
			AppWarp.WarpClient.getInstance().sendPrivateChat(user,{message : "handshake"});
			this.shakes += 1;
		}
		else
			AppWarp.WarpClient.getInstance().sendChat({message : "handshake"});
	},
